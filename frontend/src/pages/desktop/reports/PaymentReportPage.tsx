...
const PaymentReportPage = () => {
  ...
  const [pendingFilters, setPendingFilters] = useState<Partial<PaymentFilters>>({
    paymentFromDate: '',
    paymentToDate: '',
    companyId: '',
    status: 'All',
  ...
  const getContextSubtitle = (): string => {
    if (!hasFiltersApplied()) {
      return 'No filters applied. Select a date range or search to view payments.';
    }

    const parts: string[] = [];

    if (filters.paymentFromDate && filters.paymentToDate) {
      const fromDate = new Date(filters.paymentFromDate);
      const toDate = new Date(filters.paymentToDate);
    ...
    if (filters.paymentMode && filters.paymentMode !== 'All') {
      parts.push(`Mode: ${filters.paymentMode}`);
    }
  ...
  const handleGenerateWithFilters = async () => {
    ...
    try {
      const apiFilters: any = {
        ...
        paymentDateFrom: filters.paymentFromDate || undefined,
        paymentDateTo: filters.paymentToDate || undefined,
      };
      ...
      if (filters.paymentMode && filters.paymentMode !== 'All') {
        allPayments = allPayments.filter(p => p.paymentMode === filters.paymentMode);
      }

      // Sort
      allPayments.sort((a, b) => {
        const aValue = a[filters.sortBy as keyof PaymentResponse] || '';
        const bValue = b[filters.sortBy as keyof PaymentResponse] || '';
      ...
      setSummary({
        ...
        totalAmount: allPayments.reduce((sum, p) => sum + (p.paymentAmount || 0), 0),
      });
  ...
  const handleApplyFilters = async () => {
    setFilters((prev) => ({
      ...prev,
      paymentFromDate: pendingFilters.paymentFromDate || '',
      paymentToDate: pendingFilters.paymentToDate || '',
      companyId: pendingFilters.companyId || '',
      status: pendingFilters.status || 'All',
      paymentMode: pendingFilters.paymentMode || 'All',
      page: 1,
    }));
  ...
  const totalPages = dataLoaded && summary.totalPayments > 0
    ? Math.ceil(summary.totalPayments / rowsPerPage)
    : 0;
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = Math.min(startIndex + filteredPayments.length, summary.totalPayments);
          ...
          <div className="search-actions-row">
            <div className={`search-container ${loading ? 'loading' : ''}`}>
              ...
              <input
                ...
                placeholder="Search by Receipt Number, Reference Number, Company, Amount"
                value={searchQuery}
            ...
            >
              ...
              {[
                ...
                filters.paymentMode && filters.paymentMode !== 'All' ? filters.paymentMode : ''
                ...
                <div className="filter-modal-body">
                  <div className="filter-modal-grid">
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Payment Date</label>
                      <div className="filter-date-range">
                        <input
                          type="date"
                          className="filter-modal-input"
                          value={pendingFilters.paymentFromDate || ''}
                          onChange={(e) => setPendingFilters(prev => ({ ...prev, paymentFromDate: e.target.value }))}
                        ...
                        <input
                          type="date"
                          className="filter-modal-input"
                          value={pendingFilters.paymentToDate || ''}
                          onChange={(e) => setPendingFilters(prev => ({ ...prev, paymentToDate: e.target.value }))}
                          placeholder="To Date"
                        />
                      </div>
                    </div>
                    ...
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Payment Mode</label>
                      <select
                        className="filter-modal-select"
                        value={pendingFilters.paymentMode || 'All'}
                        onChange={(e) => setPendingFilters(prev => ({ ...prev, paymentMode: e.target.value }))}
                      >
          ...
          <div className="results-section" ref={resultsRef}>
                ...
                <tbody className="table-body">
                  {loading ? (
                      ...
                      <td colSpan={7} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <p className="empty-state-text">
                            Use the search bar or apply filters above to view payment reports.
                          </p>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={7} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <p className="empty-state-text">
                            {searchQuery.trim()
                              ? `No payments match "${searchQuery.trim()}". Try adjusting your search or filters.`
                              : hasFiltersApplied()
                                ? 'No payments match the current filters. Try adjusting your filter criteria.'
                                : 'No payments available. Use the search bar or apply filters to view reports.'}
                          </p>
                    ...
                    filteredPayments.map((payment, index) => (
                      <tr key={payment.paymentId} className={index % 2 === 0 ? 'table-row-even' : 'table-row-odd'}>
                        <td className="invoice-number-cell">{payment.receiptNumber || 'N/A'}</td>
                        <td>{companies.find(c => c.id === payment.companyId)?.companyName || 'N/A'}</td>
                        <td>{formatDate(payment.paymentDate)}</td>
                        <td>{payment.paymentMode}</td>
                        <td className="text-right">{formatCurrency(payment.paymentAmount)}</td>
            ...
            <div className="pagination-controls">
              <div className="pagination-info">
                {dataLoaded && filteredPayments.length > 0
                  ? `Showing ${startIndex + 1}-${endIndex} of ${summary.totalPayments} items`
              ...
              <div className="pagination-right">
                <select
                  ...
                  value={rowsPerPage}
                  disabled={!dataLoaded || filteredPayments.length === 0}
                  onChange={async (e) => {
                ...
                <div className="pagination-numbers">
                  <button
                    ...
                    disabled={currentPage === 1 || !dataLoaded || filteredPayments.length === 0}
                  ...
                  <button
                    ...
                    disabled={currentPage === totalPages || totalPages === 0 || !dataLoaded || filteredPayments.length === 0}

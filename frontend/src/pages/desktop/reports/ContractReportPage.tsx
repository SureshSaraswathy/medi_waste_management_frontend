...
const ContractReportPage = () => {
  ...
  const [pendingFilters, setPendingFilters] = useState<Partial<ContractFilters>>({
    fromDate: '',
    toDate: '',
    companyId: '',
    hcfId: '',
  ...
  const getContextSubtitle = (): string => {
    if (!hasFiltersApplied()) {
      return 'No filters applied. Select a date range or search to view contracts.';
    }
  ...
  const handleGenerateWithFilters = async () => {
    ...
    try {
      ...
      if (filters.fromDate) {
        const fromDate = new Date(filters.fromDate);
        allContracts = allContracts.filter(c => new Date(c.startDate) >= fromDate);
      }

      if (filters.toDate) {
        const toDate = new Date(filters.toDate);
        toDate.setHours(23, 59, 59, 999);
        allContracts = allContracts.filter(c => new Date(c.startDate) <= toDate);
      }

      if (filters.billingType && filters.billingType !== 'All') {
        allContracts = allContracts.filter(c => c.billingType === filters.billingType);
      }
    ...
    <AppLayout navItems={navItems}>
      ...
      <div className="invoice-report-page">
        <div className="report-breadcrumb">
          ...
          <span className="breadcrumb-separator">/</span>
          <span className="breadcrumb-item">Contract</span>
        </div>
        <div className="report-content-wrapper">
          <div className="page-header">
            <h1 className="page-title">Contract Report</h1>
            {!hasFiltersApplied() && (
          ...
          <div className="search-actions-row">
              ...
              onClick={() => {
                ...
                if (!showAdvancedFilters) {
                  setPendingFilters({
                    ...
                    billingType: filters.billingType || 'All',
                  });
                }
            ...
            <ReportExportDropdown
              ...
              onExport={(format) => {
                try {
                  reportExportService.exportAuto(filteredContracts as Record<string, any>[], format as ExportFormat, 'Contract_Report');
                ...
                <div className="filter-modal-body">
                  <div className="filter-modal-grid">
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Contract Date</label>
                    ...
                    <div className="filter-modal-group">
                      ...
                      <select
                        ...
                        onChange={(e) => setPendingFilters(prev => ({ ...prev, billingType: e.target.value }))}
                      >
                      ...
                      <td colSpan={7} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <p className="empty-state-text">
                            Use the search bar or apply filters above to view contract reports.
                          </p>
                        </div>
                      </td>
                    </tr>

...
const RevenueReportPage = () => {
  ...
  const [pendingFilters, setPendingFilters] = useState<Partial<RevenueFilters>>({
    fromDate: '',
    toDate: '',
    companyId: '',
    hcfId: '',
  ...
  const [revenueData, setRevenueData] = useState<InvoiceResponse[]>([]);
  const [filteredRevenueData, setFilteredRevenueData] = useState<InvoiceResponse[]>([]);
  ...
  const getContextSubtitle = (): string => {
    if (!hasFiltersApplied()) {
      return 'No filters applied. Select a date range or search to view revenue analysis.';
    }
    ...
    if (filters.groupBy) {
      parts.push(`Group By: ${filters.groupBy}`);
    }
  ...
  const handleGenerateWithFilters = async () => {
    ...
    try {
      ...
      if (filters.searchText?.trim()) {
        ...
        allInvoices = allInvoices.filter(invoice => {
          ...
          return invoiceNum.includes(searchLower) ||
                 ...
                 invoice.invoiceValue.toString().includes(searchLower);
        });
      }
      ...
      setRevenueData(allInvoices);
      setFilteredRevenueData(paginatedData);
      setDataLoaded(true);

      setSummary({
        totalRevenue: allInvoices.reduce((sum, inv) => sum + (inv.invoiceValue || 0), 0),
        totalInvoices: allInvoices.length,
        paidAmount: allInvoices.reduce((sum, inv) => sum + (inv.totalPaidAmount || 0), 0),
      });

    } catch (error: any) {
      console.error('Error loading revenue data:', error);
      setError(error.message || 'Failed to load revenue analysis data');
      setRevenueData([]);
  ...
  const handleApplyFilters = async () => {
    setFilters((prev) => ({
      ...
      groupBy: pendingFilters.groupBy || 'Company',
      page: 1,
    }));
  ...
  const totalPages = dataLoaded && summary.totalInvoices > 0
    ...
    : 0;
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = Math.min(startIndex + filteredRevenueData.length, summary.totalInvoices);
    ...
    <AppLayout navItems={navItems}>
      <div className="invoice-report-page">
        <div className="report-breadcrumb">
          ...
          <span className="breadcrumb-separator">/</span>
          <span className="breadcrumb-item">Revenue Analysis</span>
        </div>
        <div className="report-content-wrapper">
          <div className="page-header">
            <h1 className="page-title">Revenue Analysis Report</h1>
            {!hasFiltersApplied() && (
              ...
              onClick={() => {
                ...
                if (!showAdvancedFilters) {
                  setPendingFilters({
                    ...
                    hcfId: filters.hcfId || '',
                    groupBy: filters.groupBy || 'Company',
                  });
                }
              ...
              >
                ...
                <div className="filter-modal-body">
                  <div className="filter-modal-grid">
                    ...
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Group By</label>
                      <select
                        className="filter-modal-select"
                        value={pendingFilters.groupBy || 'Company'}
                        onChange={(e) => setPendingFilters(prev => ({ ...prev, groupBy: e.target.value }))}
                      >
          ...
          <div className="results-section" ref={resultsRef}>
              ...
              <table className="invoice-table">
                ...
                <tbody className="table-body">
                  {loading ? (
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={7} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <h3 className="empty-state-title">No revenue data found</h3>
                          <p className="empty-state-text">
                            Use the search bar or apply filters above to view revenue analysis reports.
                          </p>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={7} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <p className="empty-state-text">
                            {searchQuery.trim()
                              ? `No data match "${searchQuery.trim()}". Try adjusting your search or filters.`
                              : hasFiltersApplied()
                                ? 'No data match the current filters. Try adjusting your filter criteria.'
                                : 'No revenue data available. Use the search bar or apply filters to view reports.'}
                          </p>
                    ...
                    filteredRevenueData.map((invoice, index) => (
            ...
            <div className="pagination-controls">
              <div className="pagination-info">
                {dataLoaded && filteredRevenueData.length > 0
              ...
              <div className="pagination-right">
                <select
                  ...
                  value={rowsPerPage}
                  disabled={!dataLoaded || filteredRevenueData.length === 0}
                  onChange={async (e) => {
                ...
                <div className="pagination-numbers">
                  <button
                    ...
                    disabled={currentPage === 1 || !dataLoaded || filteredRevenueData.length === 0}
                  ...
                  <button
                    ...
                    disabled={currentPage === totalPages || totalPages === 0 || !dataLoaded || filteredRevenueData.length === 0}

...
import './agreementReportPage.css';
...
const AgreementReportPage = () => {
  ...
  const [pendingFilters, setPendingFilters] = useState<Partial<AgreementFilters>>({
    fromDate: '',
    toDate: '',
    contractId: '',
    status: 'All',
  ...
  const getContextSubtitle = (): string => {
    if (!hasFiltersApplied()) {
      return 'No filters applied. Select a date range or search to view agreements.';
    }
    ...
    if (filters.contractId) {
      ...
      if (contract) {
        parts.push(`Contract: ${contract.contractNum}`);
      }
    }
  ...
  const handleGenerateWithFilters = async () => {
    ...
    try {
      ...
      let allAgreements: AgreementResponse[] = await agreementService.getAllAgreements(apiFilters.contractId, apiFilters.status);

      // Client-side filtering
      if (filters.fromDate) {
        const fromDate = new Date(filters.fromDate);
        allAgreements = allAgreements.filter(a => new Date(a.agreementDate) >= fromDate);
      }

      if (filters.toDate) {
        const toDate = new Date(filters.toDate);
        toDate.setHours(23, 59, 59, 999);
        allAgreements = allAgreements.filter(a => new Date(a.agreementDate) <= toDate);
      }

      if (filters.searchText?.trim()) {
        const searchLower = filters.searchText.toLowerCase().trim();
        allAgreements = allAgreements.filter(agreement => {
          const agreementNum = (agreement.agreementNum || '').toLowerCase();
          const contractNum = (agreement.contractNum || '').toLowerCase();
          return agreementNum.includes(searchLower) ||
      ...
      allAgreements.sort((a, b) => {
        const aValue = a[filters.sortBy as keyof AgreementResponse] || '';
        const bValue = b[filters.sortBy as keyof AgreementResponse] || '';
      ...
      const paginatedData = allAgreements.slice(startIndex, endIndex);

      setAgreements(allAgreements);
      setFilteredAgreements(paginatedData);
      setDataLoaded(true);
      ...
      setError(error.message || 'Failed to load agreement data');
      setAgreements([]);
  ...
  const handleApplyFilters = async () => {
    setFilters((prev) => ({
      ...
      contractId: pendingFilters.contractId || '',
  ...
  useEffect(() => {
    if (contracts.length > 0 || !filters.contractId) {
  ...
  const totalPages = dataLoaded && summary.totalAgreements > 0
    ? Math.ceil(summary.totalAgreements / rowsPerPage)
    : 0;
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = Math.min(startIndex + filteredAgreements.length, summary.totalAgreements);
    ...
    <AppLayout navItems={navItems}>
      ...
      <div className="invoice-report-page">
        <div className="report-breadcrumb">
          ...
          <span className="breadcrumb-separator">/</span>
          <span className="breadcrumb-item">Agreement</span>
        </div>
        <div className="report-content-wrapper">
          <div className="page-header">
            <h1 className="page-title">Agreement Report</h1>
            {!hasFiltersApplied() && (
          ...
          <div className="search-actions-row">
            <div className={`search-container ${loading ? 'loading' : ''}`}>
              ...
              <input
                ...
                placeholder="Search by Agreement Number, Contract Number"
                value={searchQuery}
            ...
            <ReportExportDropdown
              ...
              onExport={(format) => {
                try {
                  reportExportService.exportAuto(filteredAgreements as Record<string, any>[], format as ExportFormat, 'Agreement_Report');
                ...
                <div className="filter-modal-body">
                  <div className="filter-modal-grid">
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Agreement Date</label>
                    ...
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Contract</label>
                      <select
                        className="filter-modal-select"
                        value={pendingFilters.contractId || ''}
                        onChange={(e) => setPendingFilters(prev => ({ ...prev, contractId: e.target.value }))}
                      >
          ...
          <div className="results-section" ref={resultsRef}>
                ...
                <tbody className="table-body">
                  {loading ? (
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={4} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <h3 className="empty-state-title">No agreements found</h3>
                          <p className="empty-state-text">
                            Use the search bar or apply filters above to view agreement reports.
                          </p>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={4} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <p className="empty-state-text">
                            {searchQuery.trim()
                              ? `No agreements match "${searchQuery.trim()}". Try adjusting your search or filters.`
                              : hasFiltersApplied()
                                ? 'No agreements match the current filters. Try adjusting your filter criteria.'
                                : 'No agreements available. Use the search bar or apply filters to view reports.'}
                          </p>
                    ...
                    filteredAgreements.map((agreement, index) => (
                      <tr key={agreement.id} className={index % 2 === 0 ? 'table-row-even' : 'table-row-odd'}>
                        <td className="invoice-number-cell">{agreement.agreementNum}</td>
                        <td>{agreement.contractNum || 'N/A'}</td>
            ...
            <div className="pagination-controls">
              <div className="pagination-info">
                {dataLoaded && filteredAgreements.length > 0
                  ? `Showing ${startIndex + 1}-${endIndex} of ${summary.totalAgreements} items`
              ...
              <div className="pagination-right">
                <select
                  ...
                  value={rowsPerPage}
                  disabled={!dataLoaded || filteredAgreements.length === 0}
                  onChange={async (e) => {
                ...
                <div className="pagination-numbers">
                  <button
                    ...
                    disabled={currentPage === 1 || !dataLoaded || filteredAgreements.length === 0}
                  ...
                  <button
                    ...
                    disabled={currentPage === totalPages || totalPages === 0 || !dataLoaded || filteredAgreements.length === 0}

...
const TrainingCertificateReportPage = () => {
  ...
  const [pendingFilters, setPendingFilters] = useState<Partial<TrainingFilters>>({
    trainingFromDate: '',
    trainingToDate: '',
    companyId: '',
    hcfId: '',
  ...
  const [certificates, setCertificates] = useState<TrainingCertificateResponse[]>([]);
  const [filteredCertificates, setFilteredCertificates] = useState<TrainingCertificateResponse[]>([]);
  ...
  const hasFiltersApplied = (): boolean => {
    return !!(
      filters.trainingFromDate ||
      filters.trainingToDate ||
      filters.companyId ||
      filters.hcfId ||
  ...
  const getContextSubtitle = (): string => {
    if (!hasFiltersApplied()) {
      return 'No filters applied. Select a date range or search to view training certificates.';
    }

    const parts: string[] = [];

    if (filters.trainingFromDate && filters.trainingToDate) {
      const fromDate = new Date(filters.trainingFromDate);
      const toDate = new Date(filters.trainingToDate);
  ...
  const handleGenerateWithFilters = async () => {
    ...
    try {
      const apiFilters: any = {
        ...
        dateFrom: filters.trainingFromDate || undefined,
        dateTo: filters.trainingToDate || undefined,
      };

      let allCertificates: TrainingCertificateResponse[] = await trainingCertificateService.getAllCertificates(
        apiFilters.companyId,
        false,
        apiFilters
      );
      ...
      if (filters.searchText?.trim()) {
        ...
        allCertificates = allCertificates.filter(cert => {
          ...
          const staffName = (cert.staffName || '').toLowerCase();
          const staffCode = (cert.staffCode || '').toLowerCase();
          ...
          return certNo.includes(searchLower) ||
      ...
      allCertificates.sort((a, b) => {
        const aValue = a[filters.sortBy as keyof TrainingCertificateResponse] || '';
        const bValue = b[filters.sortBy as keyof TrainingCertificateResponse] || '';
      ...
      const paginatedData = allCertificates.slice(startIndex, endIndex);

      setCertificates(allCertificates);
      setFilteredCertificates(paginatedData);
      setDataLoaded(true);
      ...
      setError(error.message || 'Failed to load training certificate data');
      setCertificates([]);
  ...
  const handleApplyFilters = async () => {
    setFilters((prev) => ({
      ...prev,
      trainingFromDate: pendingFilters.trainingFromDate || '',
      trainingToDate: pendingFilters.trainingToDate || '',
  ...
  const totalPages = dataLoaded && summary.totalCertificates > 0
    ? Math.ceil(summary.totalCertificates / rowsPerPage)
    : 0;
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = Math.min(startIndex + filteredCertificates.length, summary.totalCertificates);
    ...
    <AppLayout navItems={navItems}>
      <div className="invoice-report-page">
        <div className="report-breadcrumb">
          ...
          <span className="breadcrumb-separator">/</span>
          <span className="breadcrumb-item">Training Certificate</span>
        </div>
        <div className="report-content-wrapper">
          <div className="page-header">
            <h1 className="page-title">Training Certificate Report</h1>
            {!hasFiltersApplied() && (
          ...
          <div className="search-actions-row">
            <div className={`search-container ${loading ? 'loading' : ''}`}>
              ...
              <input
                ...
                placeholder="Search by Certificate No, Staff Name, Staff Code, Company, HCF"
                value={searchQuery}
            ...
            <button
              ...
              onClick={() => {
                ...
                if (!showAdvancedFilters) {
                  setPendingFilters({
                    trainingFromDate: filters.trainingFromDate || '',
                    trainingToDate: filters.trainingToDate || '',
                ...
                <div className="filter-modal-body">
                  <div className="filter-modal-grid">
                    <div className="filter-modal-group">
                      <label className="filter-modal-label">Training Date</label>
                      <div className="filter-date-range">
                        <input
                          type="date"
                          className="filter-modal-input"
                          value={pendingFilters.trainingFromDate || ''}
                          onChange={(e) => setPendingFilters(prev => ({ ...prev, trainingFromDate: e.target.value }))}
                        ...
                        <input
                          type="date"
                          className="filter-modal-input"
                          value={pendingFilters.trainingToDate || ''}
                          onChange={(e) => setPendingFilters(prev => ({ ...prev, trainingToDate: e.target.value }))}
                          placeholder="To Date"
                        />
                      </div>
                    </div>
          ...
          <div className="results-section" ref={resultsRef}>
                ...
                <tbody className="table-body">
                  {loading ? (
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={8} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <h3 className="empty-state-title">No training certificates found</h3>
                          <p className="empty-state-text">
                            Use the search bar or apply filters above to view training certificate reports.
                          </p>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr className="table-row-empty">
                      <td colSpan={8} className="empty-state-cell">
                        <div className="empty-state-content">
                          ...
                          <p className="empty-state-text">
                            {searchQuery.trim()
                              ? `No certificates match "${searchQuery.trim()}". Try adjusting your search or filters.`
                              : hasFiltersApplied()
                                ? 'No certificates match the current filters. Try adjusting your filter criteria.'
                                : 'No training certificates available. Use the search bar or apply filters to view reports.'}
                          </p>
                    ...
                    filteredCertificates.map((cert, index) => (
                      <tr key={cert.id} className={index % 2 === 0 ? 'table-row-even' : 'table-row-odd'}>
                        <td className="invoice-number-cell">{cert.certificateNo}</td>
                        <td>{cert.staffName}</td>
                        <td>{cert.staffCode}</td>
                        <td>{companies.find(c => c.id === cert.companyId)?.companyName || 'N/A'}</td>
                        <td>{hcfs.find(h => h.id === cert.hcfId)?.hcfCode || 'N/A'}</td>
                        <td>{formatDate(cert.trainingDate)}</td>
                        <td>{cert.trainedBy}</td>
            ...
            <div className="pagination-controls">
              <div className="pagination-info">
                {dataLoaded && filteredCertificates.length > 0
                  ? `Showing ${startIndex + 1}-${endIndex} of ${summary.totalCertificates} items`
              ...
              <div className="pagination-right">
                <select
                  ...
                  value={rowsPerPage}
                  disabled={!dataLoaded || filteredCertificates.length === 0}
                  onChange={async (e) => {
                ...
                <div className="pagination-numbers">
                  <button
                    ...
                    disabled={currentPage === 1 || !dataLoaded || filteredCertificates.length === 0}
                  ...
                  <button
                    ...
                    disabled={currentPage === totalPages || totalPages === 0 || !dataLoaded || filteredCertificates.length === 0}
...
export default TrainingCertificateReportPage;

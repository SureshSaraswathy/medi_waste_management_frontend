...
export const routeAssignmentService = {
  createRouteAssignment: async (data: CreateRouteAssignmentRequest): Promise<RouteAssignmentResponse> => {
    const response = await apiRequest<ApiResponse<RouteAssignmentResponse>>('/route-assignments', {
      method: 'POST',
  ...
  getRouteAssignmentById: async (assignmentId: string): Promise<RouteAssignmentResponse> => {
    const response = await apiRequest<ApiResponse<RouteAssignmentResponse>>(`/route-assignments/${assignmentId}`, {
      method: 'GET',
    });
    return response.data;
  },

  getAllRouteAssignments: async (
    companyId?: string,
    date?: string,
    status?: string
  ): Promise<RouteAssignmentResponse[]> => {
    let url = '/route-assignments';
    ...
    const response = await apiRequest<ApiResponse<RouteAssignmentResponse[]>>(url, {
      method: 'GET',
    });
    return response.data;
  },
  ...
  deleteRouteAssignment: async (assignmentId: string): Promise<void> => {
    await apiRequest(`/route-assignments/${assignmentId}`, {
      method: 'DELETE',
    });
  },
};
